...
EDIT_INPUT_JSON_DEFAULT = INPUT_JSON_DEFAULT.with_name(
    f"{INPUT_JSON_DEFAULT.stem}_evaluation.json"
)
EXAM_INPUT_JSON_DEFAULT = Path(r"export\datasetv3.18_hf_updated.json")
PREVIEW_WIDTH = 280
PREVIEW_HEIGHT = 220
...
EXAM_MECHANISM_OPTIONS_BY_SCENARIO = {
    "affection": [
        "multimodal incongruity",
        "figurative semantics",
        "affective deception",
        "socio_cultural context dependency",
    ],
    "attitude": [
...
EXAM_LABEL_OPTIONS_BY_SCENARIO = {
...
def _default_exam_output_json_path(input_json_path: Path) -> Path:
    if input_json_path.is_file():
        if input_json_path.stem.endswith("_exam"):
            return input_json_path
        return input_json_path.with_name(f"{input_json_path.stem}_exam.json")
    if input_json_path.is_dir():
        return input_json_path / f"{input_json_path.name}_exam.json"
    return EXAM_INPUT_JSON_DEFAULT.with_name(f"{EXAM_INPUT_JSON_DEFAULT.stem}_exam.json")
...
def _edit_process_json_path(eval_json_path: Path) -> Path:
    return eval_json_path.with_name(f"{eval_json_path.stem}_edit_process.json")


def _exam_process_json_path(output_json_path: Path) -> Path:
    if output_json_path.stem.endswith("_exam"):
        return output_json_path.with_name(f"{output_json_path.stem}_process.json")
    return output_json_path.with_name(f"{output_json_path.stem}_exam_process.json")
...
def _init_session_state() -> None:
    default_output_path = _default_exam_output_json_path(EXAM_INPUT_JSON_DEFAULT)
    default_process_path = _exam_process_json_path(default_output_path)
    defaults = {
        "current_index": 0,
        "current_failed_pos": 0,
        "is_locked": False,
        "abandon_selected": False,
        "input_json_path": str(EXAM_INPUT_JSON_DEFAULT),
        "active_input_json_path": str(EXAM_INPUT_JSON_DEFAULT),
        ...
        "last_loaded_edit_sample_key": "",
        "last_loaded_exam_sample_key": "",
        "exam_mechanism_choice": "",
        "exam_label_choice": "",
        "exam_subject_choice": "",
        "exam_target_choice": "",
    }
...
def _edit_key(field: str) -> str:
    return f"edit__{field}"


def _is_failed_item(item: Dict[str, Any]) -> bool:
    output = item.get("output", {}) if isinstance(item.get("output"), dict) else {}
    abandon_raw = _parse_bool(output.get("abandon"))
    is_abandon = bool(abandon_raw) if abandon_raw is not None else False
    if is_abandon:
        return False
    evaluation = _normalize_evaluation(output.get("evaluation"))
    failed_fields = evaluation.get("failed_fields", [])
    if failed_fields:
        return True
    if bool(evaluation.get("evaluated")) and evaluation.get("overall_pass") is False:
        return True
    return False


def _failed_item_indices(items: List[Dict[str, Any]]) -> List[int]:
    out: List[int] = []
...
def _save_edit_process_json(
    ...
    data = {
        ...
        "resolved_failed": max(0, len(items) - unresolved),
...
def _load_edit_process_item_index(process_path: Path, total: int) -> int:
    if not process_path.exists():
        return 0
    try:
        data = json.loads(process_path.read_text(encoding="utf-8"))
        idx = int(data.get("current_item_index", 0))
        if total <= 0:
            return 0
        return max(0, min(idx, total - 1))
    except Exception:
...
def _load_edit_field_state(item: Dict[str, Any], sample_key: str) -> None:
    if st.session_state.get("last_loaded_edit_sample_key", "") == sample_key:
        return

    output = item.get("output", {}) if isinstance(item.get("output"), dict) else {}

    for field in EVAL_FIELDS:
        state_key = _edit_key(field)
        raw_value = _safe_text(output.get(field, "")).strip()
        if field == "scenario":
            st.session_state[state_key] = _safe_choice(
                _norm_scenario(raw_value), SCENARIO_OPTIONS, allow_empty=False
            )
        elif field == "mechanism_Affection":
            st.session_state[state_key] = _safe_choice(
                _norm_mechanism_value(raw_value),
                MECHANISM_AFFECTION_OPTIONS,
                allow_empty=False,
            )
        elif field == "mechanism_Intent":
            st.session_state[state_key] = _safe_choice(
                _norm_mechanism_value(raw_value),
                MECHANISM_INTENT_OPTIONS,
                allow_empty=False,
            )
        elif field == "mechanism_Attitude":
            st.session_state[state_key] = _safe_choice(
                _norm_mechanism_value(raw_value),
                MECHANISM_ATTITUDE_OPTIONS,
                allow_empty=False,
            )
        elif field == "label_Affection":
            st.session_state[state_key] = _safe_choice(
                _norm_affection_label(raw_value), Affection_OPTIONS, allow_empty=True
            )
        elif field == "label_Intent":
            st.session_state[state_key] = _safe_choice(
                _norm_intent_label(raw_value), INTENT_OPTIONS, allow_empty=False
            )
        elif field == "label_Attitude":
            st.session_state[state_key] = _safe_choice(
                raw_value, ATTITUDE_OPTIONS, allow_empty=True
            )
        else:
            st.session_state[state_key] = raw_value

    st.session_state.last_loaded_edit_sample_key = sample_key


def _collect_edit_output_values() -> Dict[str, str]:
    out: Dict[str, str] = {}
    for field in EVAL_FIELDS:
        out[field] = _safe_text(st.session_state.get(_edit_key(field), "")).strip()
    return out


def _apply_edited_output_values(item: Dict[str, Any]) -> None:
    output = item.setdefault("output", {})
    merged_output = dict(output)
    merged_output.update(_collect_edit_output_values())
    normalized_output = _normalize_output(merged_output)
    normalized_output["evaluation"] = _normalize_evaluation(output.get("evaluation"))
    abandon_raw = _parse_bool(output.get("abandon"))
    normalized_output["abandon"] = bool(abandon_raw) if abandon_raw is not None else False
    item["output"] = normalized_output


def _field_label(field: str) -> str:
    if field == "label_Affection":
        return "Label: Affection"
    if field == "label_Intent":
        return "Label: Intent"
    if field == "label_Attitude":
        return "Label: Attitude"
    if field == "mechanism_Affection":
        return "Mechanism: Affection"
    if field == "mechanism_Intent":
        return "Mechanism: Intent"
    if field == "mechanism_Attitude":
        return "Mechanism: Attitude"
    if field == "scenario":
        return "Scenario"
    return field


def _render_edit_field_row(field: str) -> None:
    state_key = _edit_key(field)
    ...
    st.checkbox("Mark Fail", key=_failed_key(field))


def _next_failed_position_after_submit(
    items: List[Dict[str, Any]], current_item_index: int
) -> int:
    failed_indices = _failed_item_indices(items)
    if not failed_indices:
        return 0
    for pos, idx in enumerate(failed_indices):
        if idx > current_item_index:
            return pos
    return 0


def _run_json_edit_ui() -> None:
    import_cols = st.columns([0.75, 0.25], gap="small")
    with import_cols[0]:
        st.text_input("Evaluation JSON path", key="input_json_path")
    with import_cols[1]:
        load_clicked = st.button("load_file", use_container_width=True, type="primary")

    if load_clicked:
        try:
            eval_json_path = Path(_safe_text(st.session_state.input_json_path)).expanduser()
            items = _load_json_items(eval_json_path)
            if not items:
                raise ValueError("No valid records found in evaluation JSON.")
            process_json_path = _edit_process_json_path(eval_json_path)
            st.session_state.active_input_json_path = str(eval_json_path)
            st.session_state.output_json_path = str(eval_json_path)
            st.session_state.process_json_path = str(process_json_path)
            st.session_state.last_loaded_output_path = ""
            st.session_state.last_loaded_sample_key = ""
            st.session_state.last_loaded_edit_sample_key = ""
            st.session_state.current_failed_pos = 0
            failed_count = len(_failed_item_indices(items))
            st.session_state.last_import_summary = (
                f"Loaded: {len(items)} | Failed unresolved: {failed_count}"
            )
    ...
    eval_json_path = Path(_safe_text(st.session_state.output_json_path))
    process_json_path = Path(_safe_text(st.session_state.process_json_path))

    items = _load_json_items(eval_json_path)
    if not items:
        st.warning("No records loaded. Please provide an evaluation JSON file and click `load_file`.")
        st.stop()
    ...
    if st.session_state.get("last_loaded_output_path", "") != str(eval_json_path):
        resume_item_index = _load_edit_process_item_index(process_json_path, total)
        if resume_item_index in failed_indices:
            st.session_state.current_failed_pos = failed_indices.index(resume_item_index)
        else:
            st.session_state.current_failed_pos = 0
        st.session_state.last_loaded_output_path = str(eval_json_path)
        st.session_state.last_loaded_sample_key = ""
        st.session_state.last_loaded_edit_sample_key = ""

    if failed_total == 0:
        st.success("No unresolved failed samples. All evaluated failures have been fixed.")
        _save_edit_process_json(
            process_path=process_json_path,
    ...
    current_failed_pos = int(st.session_state.get("current_failed_pos", 0))
    current_failed_pos = max(0, min(current_failed_pos, failed_total - 1))
    st.session_state.current_failed_pos = current_failed_pos
    current_index = failed_indices[current_failed_pos]
    ...
    _load_edit_field_state(current_item, sample_key)
    ...
    with left:
        ...
        st.divider()
        st.caption("Current failed fields")
        failed_now = _selected_failed_fields()
        if failed_now:
            st.markdown(" | ".join([f"`{x}`" for x in failed_now]))
        else:
            st.caption("No field marked failed.")

        st.markdown("**Field columns 1-2**")
        left_field_cols = st.columns(2, gap="small")
        with left_field_cols[0]:
            for field in EVAL_COLUMN_1_FIELDS:
                _render_edit_field_row(field)
        with left_field_cols[1]:
            for field in EVAL_COLUMN_2_FIELDS:
                _render_edit_field_row(field)

    with right:
        pos_prog = (current_failed_pos + 1) / failed_total if failed_total else 0.0
        st.progress(pos_prog)
        st.caption(
            f"Failed queue: {current_failed_pos + 1}/{failed_total} | Total samples: {total}"
        )
        ...
        with jump_cols[0]:
            jump_page = st.number_input(
                "Failed Page",
                min_value=1,
                max_value=failed_total,
                value=current_failed_pos + 1,
                step=1,
                key="jump_page_input",
            )
        with jump_cols[1]:
        ...
        accept_clicked = st.button(
            ...
            type="primary",
            disabled=bool(st.session_state.abandon_marked) or bool(_selected_failed_fields()),
        )

        st.caption(f"Editing: `{eval_json_path}`")
        st.caption("Edit fields and keep F checked only for fields that still fail.")

        st.markdown("**Field columns 3-4**")
        right_field_cols = st.columns(2, gap="small")
    ...
    def _go_failed(pos: int) -> None:
        failed_after = _failed_item_indices(items)
        if not failed_after:
            st.session_state.current_failed_pos = 0
            _save_edit_process_json(
                process_path=process_json_path,
                eval_json_path=eval_json_path,
                items=items,
                failed_indices=failed_after,
                current_item_index=0,
            )
            _rerun()
        bounded_pos = max(0, min(pos, len(failed_after) - 1))
        st.session_state.current_failed_pos = bounded_pos
        _save_edit_process_json(
            process_path=process_json_path,
            eval_json_path=eval_json_path,
            items=items,
            failed_indices=failed_after,
            current_item_index=failed_after[bounded_pos],
        )
        _rerun()

    if jump_clicked:
        _go_failed(int(jump_page) - 1)

    if prev_clicked:
        _go_failed(current_failed_pos - 1)

    if save_next_clicked:
        _apply_edited_output_values(current_item)
        ...
        _save_json_items(eval_json_path, items)
        st.session_state.last_loaded_sample_key = ""
        st.session_state.last_loaded_edit_sample_key = ""
        next_pos = _next_failed_position_after_submit(items, current_index)
        st.session_state.current_failed_pos = next_pos
        failed_after_submit = _failed_item_indices(items)
        current_item_index_after = (
            failed_after_submit[next_pos] if failed_after_submit else 0
        )
        _save_edit_process_json(
            process_path=process_json_path,
            eval_json_path=eval_json_path,
            items=items,
            failed_indices=failed_after_submit,
            current_item_index=current_item_index_after,
        )
        _rerun()
    ...
    if accept_clicked:
        ...
        next_pos = _next_failed_position_after_submit(items, current_index)
        st.session_state.current_failed_pos = next_pos
        failed_after_accept = _failed_item_indices(items)
        current_item_index_after = (
            failed_after_accept[next_pos] if failed_after_accept else 0
        )
        _save_edit_process_json(
            process_path=process_json_path,
            eval_json_path=eval_json_path,
            items=items,
            failed_indices=failed_after_accept,
            current_item_index=current_item_index_after,
        )
        _rerun()


def _exam_media_obj(input_obj: Dict[str, Any]) -> Dict[str, Any]:
    media = input_obj.get("media")
    if isinstance(media, dict):
        return media
    return {}


def _exam_media_url_from_input(input_obj: Dict[str, Any]) -> str:
    media_obj = _exam_media_obj(input_obj)
    for key in ("image_url", "video_url", "media_url", "url"):
        value = _safe_text(media_obj.get(key, "")).strip()
        if value:
            return value
    return _media_url_from_input(input_obj)


def _exam_filename_from_input(input_obj: Dict[str, Any]) -> str:
    media_obj = _exam_media_obj(input_obj)
    for key in ("filename", "file_name", "image_file", "video_file", "media_file"):
        value = _safe_text(input_obj.get(key, "")).strip()
        if value:
    ...
    for key in ("image_path", "video_path", "media_path_local", "path"):
        value = _safe_text(media_obj.get(key, "")).strip()
        if value:
            name = Path(value).name
            if name:
    ...
    return _filename_from_input(input_obj)


def _normalize_exam_options(options_obj: Any) -> Dict[str, List[str]]:
    subject_options: List[str] = []
    target_options: List[str] = []
    if isinstance(options_obj, dict):
        raw_subject = options_obj.get("subject", [])
        raw_target = options_obj.get("target", [])
        if isinstance(raw_subject, list):
            for value in raw_subject:
                text = _safe_text(value).strip()
                if text:
                    subject_options.append(text)
        if isinstance(raw_target, list):
            for value in raw_target:
                text = _safe_text(value).strip()
                if text:
                    target_options.append(text)
    return {
...
def _exam_scenario_key(input_obj: Dict[str, Any]) -> str:
    return _safe_text(input_obj.get("scenario", "")).strip().lower()


def _exam_mechanism_options(input_obj: Dict[str, Any]) -> List[str]:
    return EXAM_MECHANISM_OPTIONS_BY_SCENARIO.get(_exam_scenario_key(input_obj), [])


def _exam_label_options(input_obj: Dict[str, Any]) -> List[str]:
    return EXAM_LABEL_OPTIONS_BY_SCENARIO.get(_exam_scenario_key(input_obj), [])


def _normalize_exam_item(item: Dict[str, Any]) -> Dict[str, Any]:
    normalized = dict(item)
    input_obj = normalized.get("input")
    if not isinstance(input_obj, dict):
        input_obj = {}
    options_obj = _normalize_exam_options(normalized.get("options"))

    output_obj = normalized.get("output")
    if not isinstance(output_obj, dict):
        output_obj = {}
    mechanism_options = _exam_mechanism_options(input_obj)
    label_options = _exam_label_options(input_obj)
    answer_mechanism = _safe_text(output_obj.get("mechanism", "")).strip()
    answer_label = _safe_text(output_obj.get("label", "")).strip()
    answer_subject = _safe_text(output_obj.get("subject", "")).strip()
    answer_target = _safe_text(output_obj.get("target", "")).strip()

    normalized["id"] = _safe_text(
        normalized.get("id", input_obj.get("id", input_obj.get("samples_id", "")))
    ).strip()
    normalized["input"] = input_obj
    normalized["options"] = options_obj
    # Exam output only stores selected options.
    normalized["output"] = {
        "mechanism": answer_mechanism if answer_mechanism in mechanism_options else "",
        "label": answer_label if answer_label in label_options else "",
        "subject": answer_subject if answer_subject in options_obj.get("subject", []) else "",
        "target": answer_target if answer_target in options_obj.get("target", []) else "",
    }
    return normalized


def _exam_item_key(item: Dict[str, Any]) -> Tuple[str, str, str]:
    item_id = _safe_text(item.get("id", "")).strip()
    input_obj = item.get("input", {}) if isinstance(item.get("input"), dict) else {}
    filename = _exam_filename_from_input(input_obj).strip()
    input_text = _safe_text(input_obj.get("text", "")).strip()
    return item_id, filename, input_text


def _load_exam_json_items(path: Path) -> List[Dict[str, Any]]:
    if not path.exists():
        return []
    raw_items = _iter_input_objects(path)
    out: List[Dict[str, Any]] = []
    for raw in raw_items:
        out.append(_normalize_exam_item(raw))
    return out


def _save_exam_json_items(path: Path, items: List[Dict[str, Any]]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(items, ensure_ascii=False, indent=2), encoding="utf-8")


def _merge_exam_items_keep_answers(
    new_items: List[Dict[str, Any]],
    existing_items: List[Dict[str, Any]],
) -> List[Dict[str, Any]]:
    existing_map: Dict[Tuple[str, str, str], Dict[str, Any]] = {}
    for item in existing_items:
        key = _exam_item_key(item)
        if key not in existing_map:
            existing_map[key] = item
    ...
    for item in new_items:
        ...
        if existing is not None:
            ...
            if isinstance(old_output, dict):
                item["output"] = {
                    "mechanism": _safe_text(old_output.get("mechanism", "")).strip(),
                    "label": _safe_text(old_output.get("label", "")).strip(),
                    "subject": _safe_text(old_output.get("subject", "")).strip(),
                    "target": _safe_text(old_output.get("target", "")).strip(),
                }
        merged.append(item)
...
def _count_exam_answered(items: List[Dict[str, Any]]) -> int:
    count = 0
    for item in items:
        output = item.get("output", {}) if isinstance(item.get("output"), dict) else {}
        mechanism = _safe_text(output.get("mechanism", "")).strip()
        label = _safe_text(output.get("label", "")).strip()
        subject = _safe_text(output.get("subject", "")).strip()
        target = _safe_text(output.get("target", "")).strip()
        if mechanism and label and subject and target:
            count += 1
    return count


def _save_exam_process_json(
    process_path: Path,
    input_json_path: Path,
    output_json_path: Path,
    items: List[Dict[str, Any]],
    ...
    if total > 0:
        ...
        current_index = 0
    answered = _count_exam_answered(items)
    data = {
        "input_json": str(input_json_path),
        "output_json": str(output_json_path),
        "total": total,
...
def _load_exam_choice_state(item: Dict[str, Any], sample_key: str) -> None:
    if st.session_state.get("last_loaded_exam_sample_key", "") == sample_key:
        return
    input_obj = item.get("input", {}) if isinstance(item.get("input"), dict) else {}
    options_obj = item.get("options", {}) if isinstance(item.get("options"), dict) else {}
    mechanism_options = _exam_mechanism_options(input_obj)
    label_options = _exam_label_options(input_obj)
    subject_options = options_obj.get("subject", []) if isinstance(options_obj.get("subject"), list) else []
    target_options = options_obj.get("target", []) if isinstance(options_obj.get("target"), list) else []
    output_obj = item.get("output", {}) if isinstance(item.get("output"), dict) else {}

    selected_mechanism = _safe_text(output_obj.get("mechanism", "")).strip()
    selected_label = _safe_text(output_obj.get("label", "")).strip()
    selected_subject = _safe_text(output_obj.get("subject", "")).strip()
    selected_target = _safe_text(output_obj.get("target", "")).strip()

    st.session_state.exam_mechanism_choice = (
        selected_mechanism if selected_mechanism in mechanism_options else ""
    )
    st.session_state.exam_label_choice = (
        selected_label if selected_label in label_options else ""
    )
    st.session_state.exam_subject_choice = (
        selected_subject if selected_subject in subject_options else ""
    )
    st.session_state.exam_target_choice = (
        selected_target if selected_target in target_options else ""
    )
    st.session_state.last_loaded_exam_sample_key = sample_key


def _apply_exam_answer(item: Dict[str, Any]) -> None:
    mechanism = _safe_text(st.session_state.get("exam_mechanism_choice", "")).strip()
    label = _safe_text(st.session_state.get("exam_label_choice", "")).strip()
    subject = _safe_text(st.session_state.get("exam_subject_choice", "")).strip()
    target = _safe_text(st.session_state.get("exam_target_choice", "")).strip()
    item["output"] = {
        "mechanism": mechanism,
        "label": label,
        "subject": subject,
...
def _start_exam_from_json(input_json_path: Path) -> Dict[str, Any]:
    raw_items = _iter_input_objects(input_json_path)
    normalized_items: List[Dict[str, Any]] = []
    skipped_no_input = 0
    for raw in raw_items:
        item = _normalize_exam_item(raw)
        if not isinstance(item.get("input"), dict):
            skipped_no_input += 1
            continue
        normalized_items.append(item)

    output_json_path = _default_exam_output_json_path(input_json_path)
    process_json_path = _exam_process_json_path(output_json_path)
    existing_items = _load_exam_json_items(output_json_path)
    merged_items = _merge_exam_items_keep_answers(normalized_items, existing_items)
    _save_exam_json_items(output_json_path, merged_items)
    ...
    for item in merged_items:
        ...
        if not isinstance(input_obj, dict):
            continue
        filename = _exam_filename_from_input(input_obj).strip()
        media_url = _exam_media_url_from_input(input_obj).strip()
        if filename and media_url and filename not in filename_to_url:
            filename_to_url[filename] = media_url
...
def _run_json_exam_ui() -> None:
    import_cols = st.columns([0.75, 0.25], gap="small")
    with import_cols[0]:
        st.text_input("Input JSON/JSONL path", key="input_json_path")
    with import_cols[1]:
        start_import_clicked = st.button("start_exam", use_container_width=True, type="primary")

    if start_import_clicked:
        try:
            input_json_path = Path(_safe_text(st.session_state.input_json_path)).expanduser()
            with st.spinner("Preparing exam data and downloading media..."):
                result = _start_exam_from_json(input_json_path)
            ...
            st.success("Exam data ready.")
        except Exception as exc:
            st.error(f"Exam start failed: {exc}")
    ...
    items = _load_exam_json_items(output_json_path)
    total = len(items)
    if total == 0:
        st.warning("No records loaded. Please set input JSON and click `start_exam`.")
        st.stop()
    ...
    input_obj = current_item.get("input", {}) if isinstance(current_item.get("input"), dict) else {}
    item_id = _safe_text(current_item.get("id", "")).strip()
    input_text = _safe_text(input_obj.get("text", "")).strip()
    filename = _exam_filename_from_input(input_obj).strip()
    sample_key = f"{current_index}|{item_id}|{filename}|{input_text}"
    _load_exam_choice_state(current_item, sample_key)

    answered = _count_exam_answered(items)
    scenario = _safe_text(input_obj.get("scenario", "")).strip()
    mechanism_options = _exam_mechanism_options(input_obj)
    label_options = _exam_label_options(input_obj)
    options_obj = current_item.get("options", {}) if isinstance(current_item.get("options"), dict) else {}
    subject_options = options_obj.get("subject", []) if isinstance(options_obj.get("subject"), list) else []
    target_options = options_obj.get("target", []) if isinstance(options_obj.get("target"), list) else []
    ...
    with left:
        ...
        with media_col:
            if filename:
                ...
                st.warning("Missing filename for this item.")
            st.caption(f"Current file: `{filename or '(none)'}`")
            st.caption(f"Scenario: `{scenario or '(none)'}`")
    ...
    with right:
        ...
        st.progress(pos_prog)
        st.caption(f"Position: {current_index + 1}/{total} | Answered: {answered}/{total}")
        ...
        st.caption("Choose mechanism, label, one subject option, and one target option.")

        # Only options are provided in exam mode.
        mechanism_choices = [""] + mechanism_options
        label_choices = [""] + label_options
        subject_choices = [""] + subject_options
        target_choices = [""] + target_options
        st.selectbox(
            "Mechanism Option",
            mechanism_choices,
            key="exam_mechanism_choice",
            format_func=lambda x: x if x else "-- Select Mechanism --",
        )
        st.selectbox(
            "Label Option",
            label_choices,
            key="exam_label_choice",
            format_func=lambda x: x if x else "-- Select Label --",
        )
        st.selectbox(
            "Subject Option",
            subject_choices,
            key="exam_subject_choice",
            format_func=lambda x: x if x else "-- Select Subject --",
        )
        st.selectbox(
            "Target Option",
            target_choices,
            key="exam_target_choice",
            format_func=lambda x: x if x else "-- Select Target --",
        )
    ...
    if save_next_clicked:
        _apply_exam_answer(current_item)
        _save_exam_json_items(output_json_path, items)
        st.session_state.last_loaded_exam_sample_key = ""
...
def main() -> None:
    st.set_page_config(page_title="Media Exam Tool", layout="wide")
