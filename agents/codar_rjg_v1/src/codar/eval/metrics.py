...
def _extract_prediction_payload(row: Dict[str, Any]) -> Dict[str, Any]:
...
def compute_metrics(gt_by_id: Dict[str, Dict[str, Any]], predictions: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for row in predictions:
        ...
        if sid not in gt_by_id:
            continue
        gt = gt_by_id[sid]
        pred = _extract_prediction_payload(row)
        total += 1
        sc = gt["scenario"]
        per_scenario[sc]["total"] += 1
        flags = {}
        for key in ("subject", "target"):
            ok = normalize_text(pred.get(key, "")) == normalize_text(gt.get(key, ""))
            flags[key] = ok
            if ok:
                correct[key] += 1
                per_scenario[sc][key] += 1
        for key in ("mechanism", "label"):
            ok = normalize_taxonomy_value(pred.get(key, "")) == normalize_taxonomy_value(gt.get(key, ""))
            flags[key] = ok
            if ok:
                correct[key] += 1
                per_scenario[sc][key] += 1
